  ...
  private async fetchUpdates(sinceTimestamp: number): Promise<any[]> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        // Handle expected "ENOTFOUND" error for the fake API
        if (error.code === 'ENOTFOUND') {
          logger.warn('Simulation Mode: Marketplace B API is unreachable (Mocking empty response)', {
            url: this.MARKETPLACE_B_API
          });
          return []; // Return empty array to simulate "no new items"
        }
